...
public class Arm extends SubsystemBase
{
  ...
  public void moveARMWithJoysticks(XboxController joystick)
  {
    double yARMValue = 0.0;
    double motorOutput = 0.0;
    double manualSpeedMax = ARMConsts.kSpeedMaxManual;

    yARMValue = -joystick.getLeftY( );
    if (yARMValue > -m_stickDeadband && yARMValue < m_stickDeadband)
    {
      if (m_mode != ARMMode.ARM_STOPPED)
        DataLogManager.log(getSubsystem( ) + "ARM Climber Stopped");
      m_mode = ARMMode.ARM_STOPPED;
    }
    else
    {
      // If joystick is above a value, climber will move up
      if (yARMValue > m_stickDeadband)
      {
        if (m_mode != ARMMode.ARM_UP)
          DataLogManager.log(getSubsystem( ) + ("ARM Climber Up"));
        m_mode = ARMMode.ARM_UP;

        yARMValue -= m_stickDeadband;
        yARMValue *= (1.0 / (1.0 - m_stickDeadband));
        motorOutput = manualSpeedMax * (yARMValue * Math.abs(yARMValue));
      }
      // If joystick is below a value, climber will move down
      else if (yARMValue < -m_stickDeadband)
      {
        if (m_mode != ARMMode.ARM_DOWN)
          DataLogManager.log(getSubsystem( ) + "ARM Climber Down");
